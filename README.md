...
| `runtime-token --env=<env>` | Mint the `VSYNC_CONFIG` bootstrap blob (a `vsync-cfg-v1:<gz+base64url>` string) for a runtime read library — pipe the only-line of stdout into your secret store. Flags: `--access-key`, `--secret-key`, `--bucket`, `--endpoint`, `--region`, `--prefix`, `--profile=<name>` to fill defaults from a saved profile, `--no-validate` to skip the manifest HEAD probe, `--json` to also dump the pre-encoding JSON to stderr (contains the secret key — debug only). |
| `rotate-passphrase --env=<env>` | Re-encrypt the latest bundle under a new passphrase and swap the pointer atomically, with a `meta.gen` counter bumped on the manifest meta cell and an `action="rotate"` row appended to the audit log. Flags: `--new-passphrase=<...>`, `--no-audit`, `--note=<text>`, `--meta key=value` (repeatable). Old passphrase is prompted interactively (never accepted on the command line). |
