...
| `salt` | string | yes | PBKDF2-SHA256 salt as **standard base64** of the raw bytes PBKDF2 sees. The CLI stores `cfg.encryption.salt` as a 24-char base64url ASCII string and feeds its UTF-8 bytes to PBKDF2 (no decode); `runtime-token` therefore encodes those UTF-8 bytes here as standard base64 (`Buffer.from(saltString, "utf8").toString("base64")`). Readers MUST base64-decode and feed the raw bytes directly to PBKDF2 — byte-identical to the CLI's input. Length is variable (24 bytes for vsync v0.x-init'd envs). |
