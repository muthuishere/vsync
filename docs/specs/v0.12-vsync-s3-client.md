...
const gen    = v.generation();                   // → number (gen at open time)
const remote = await v.remoteGeneration();       // → number (HEAD manifest; rejects on network fail)
const stale  = await v.hasNewVersion();          // → boolean (convenience: remote > local)
await v.close();
```

- `open()` is async (the S3 round-trip and decrypt are awaited). `get`/`has`/`source`/`assetBytes`/`generation` are sync (in-memory). `assetPath` is async (writes the tempfile). `remoteGeneration` / `hasNewVersion` are async (one HEAD on the manifest). `close` is async (best-effort buffer zeroing + tempfile unlink).
...
gen    := v.Generation()                 // int64 (gen at open time)
remote, err := v.RemoteGeneration(ctx)   // int64 (HEAD manifest; ErrS3Unreachable on network fail)
stale,  err := v.HasNewVersion(ctx)      // bool (convenience: remote > local)
```

- Explicit `context.Context` on `Open`, `RemoteGeneration`, and `HasNewVersion` (cancellation + deadline for the S3 fetch). Pure-memory accessors (`Get`/`Has`/`Source`/`AssetBytes`/`Generation`) don't take a context.
...
try (Vsync v = VsyncClient.open()) {
    ...
    byte[]  bytes  = v.assetBytes("service-account.json");
    String  path   = v.assetPath("service-account.json");
    long    gen    = v.generation();                  // → long (gen at open time)
    long    remote = v.remoteGeneration();            // → long (HEAD manifest; S3UnreachableException on fail)
    boolean stale  = v.hasNewVersion();               // → boolean (convenience: remote > local)
}
```
