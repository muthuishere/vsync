...
|---|---|
| **0.7.1** | `vsync sync <env> all` removed. Only `gh` and `gcp` remain as targets; one target per invocation. If you need both, run two commands. Same no-magic theme as v0.7's parser refactor — the operator names what runs. CLI rejects `all` as an unknown target. |
